...
// they run concurrently rather than costing two round-trips in sequence.
...
//
// The probes are independent and there are twelve of them, so they run
// concurrently. Sequentially, a domain publishing no DKIM at all — the common
// case, since the selector list is guesswork — paid ten round-trips to learn
// nothing, and a resolver that blackholes unknown names charged the full
// timeout for each. Answers are collected by index so the report keeps its
// fixed SPF, DMARC, selector order.
